...
class NsxPolicyLibTestCase(unittest.TestCase):
    ...
    def assert_called_with_defs(self, mock_api, expected_defs, call_num=0):
        ...
        self.assert_called_with_def(mock_api, expected_defs[0][0],
                                    call_num=call_num)
    ...
    def assert_called_with_def_and_dict(self, mock_api,
        ...
        actual_dict = mock_api.call_args_list[call_num][0][1]
...
class TestPolicyContract(NsxPolicyLibTestCase):
    ...
    def test_get_by_name(self):
        ...
        with mock.patch.object(
            ...
            obj = self.resourceApi.get_by_name(name, tenant=TEST_TENANT)
            self.assertIsNotNone(obj)
            expected_def = policy_defs.ContractDef(tenant=TEST_TENANT)
            self.assert_called_with_def(api_call, expected_def)
    ...
    def test_update_entry(self):
        ...
        service_id = '333'
        action = 'deny'
        contract_entry_id = '222'
        contract_entry = {'id': contract_entry_id}

        with mock.patch.object(
            self.policy_api, "get",
            return_value={'contract_entries': [contract_entry]}) as get_call,\
            mock.patch.object(self.policy_api, "update") as update_call:
            self.resourceApi.update(id,
                                    services=[service_id],
                                    action=action,
            ...
            # update will be called for the service entry only
            expected_entry_def = policy_defs.ContractEntryDef(
                contract_id=id,
                contract_entry_id=contract_entry_id,
                tenant=TEST_TENANT)
            expected_entry_dict = {'id': contract_entry_id,
                                   'action': action.upper(),
                                   'services': [service_id]}
            self.assert_called_with_def_and_dict(
                update_call, expected_entry_def, expected_entry_dict)
    ...
    def test_get_by_name(self):
        ...
        with mock.patch.object(
            ...
            obj = self.resourceApi.get_by_name(domain_id, name,
                                               tenant=TEST_TENANT)
            self.assertIsNotNone(obj)
            expected_def = policy_defs.ContractMapDef(domain_id,
                                                      tenant=TEST_TENANT)
            self.assert_called_with_def(api_call, expected_def)
    ...
    def test_update(self):
        ...
        with mock.patch.object(self.policy_api, "get",
            ...
            expected_def = policy_defs.ContractMapDef(domain_id=domain_id,
                                                      contractmap_id=id,
                                                      tenant=TEST_TENANT)
            sgroup_path = "/tenants/%s/domains/%s/groups/%s" % (
                TEST_TENANT, domain_id, source_group)
            dgroup_path = "/tenants/%s/domains/%s/groups/%s" % (
                TEST_TENANT, domain_id, dest_group)
            contract_path = "/tenants/%s/contracts/%s" % (
                TEST_TENANT, contract_id)
            expected_dict = {'display_name': name,
                             'description': description,
