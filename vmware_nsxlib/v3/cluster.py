...
class NSXRequestsHTTPProvider(AbstractHTTPProvider):
    ...
    def new_connection(self, cluster_api, provider):
        ...
        if config.client_cert_provider:
            session.cert = config.client_cert_provider.filename()
        else:
    ...
    def _init_endpoints(self, providers,
        ...
        def _create_conn(p):
            def _conn():
                ...
                config = self.nsxlib_config
                if config.client_cert_provider is None:
                    return self._http_provider.new_connection(self, p)

                with config.client_cert_provider:
                    # with statement here allows nsxlib user to prepare
                    # certificate and private key file and dispose it
                    # once connections are spawned (since PJK is sensitive
                    # information, immediate disposal is important)
                    return self._http_provider.new_connection(self, p)
