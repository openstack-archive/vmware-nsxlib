...
class NsxLibFirewallSection(utils.NsxLibApiBase):
    ...
    def _decide_service(self, sg_rule):
        ...
        if l4_protocol in [consts.TCP, consts.UDP]:
            ...
            if (icmp_type and consts.FEATURE_ICMP_STRICT and
                    icmp_type not in constants.IPV4_ICMP_STRICT_TYPES):
                raise exceptions.InvalidInput(
                    operation='create_rule',
                    arg_val=icmp_type,
                    arg_name='icmp_type')
            if icmp_type and icmp_type not in constants.IPV4_ICMP_TYPES:
                    raise exceptions.InvalidInput(
                        ...
                        arg_val=icmp_type,
                        arg_name='icmp_type')
            if (icmp_code and consts.FEATURE_ICMP_STRICT and icmp_type not in
                    constants.IPV4_ICMP_STRICT_TYPES[icmp_type]):
                raise exceptions.InvalidInput(
                    operation='create_rule',
                    arg_val=icmp_code,
                    arg_name='icmp_code for this icmp_type')
            if (icmp_code not in constants.IPV4_ICMP_TYPES[icmp_type]):
                    raise exceptions.InvalidInput(
                        operation='create_rule',
                        arg_val=icmp_code,
                        arg_name='icmp_code for this icmp_type')
