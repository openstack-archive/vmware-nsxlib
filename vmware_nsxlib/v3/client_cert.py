...
def validate_cert_params(key_size, valid_for_days,
    ...
    if key_size not in expected_key_sizes:
        raise exceptions.InvalidInput(
            error_message='Invalid key size %(value)d'
    ...
    if signature_alg not in expected_signature_algs:
        raise exceptions.InvalidInput(
            error_message='Invalid signature algorithm %(value)s'
                          '(must be one of %(list)s)' %
                          {'value': signature_alg,
...
class ClientCertificateManager(object):
    ...
    def delete(self):
        ...
        if details['certificate_id']:
            self._nsx_trust_management.delete_cert(details['certificate_id'])
    ...
    def export_pem(self, filename):
        ...
        if not self.exists():
            LOG.ERROR("No certificate present - nothing to export")
            return
    ...
    def expires_on(self):
        ...
        converted = datetime.datetime.strptime(self._cert.get_notAfter(),
                                               "%Y%m%d%H%M%SZ")
        return converted
