...
class NsxLibLogicalRouter(utils.NsxLibApiBase):
    ...
    def _delete_resource_by_values(self, resource,
        ...
        found.
        If issue_warnings is True - warnings will be issued if 0 or >1 objects
        where deleted.
        """
        ...
        if matched_num == 0:
            if skip_not_found:
                ...
                raise exceptions.ResourceNotFound(
                    ...
                    operation=err_msg)
        elif matched_num > 1 and issue_warnings:
    ...
    def delete_nat_rule_by_values(self, logical_router_id,
                                  delete_all=False, **kwargs):
        resource = 'logical-routers/%s/nat/rules' % logical_router_id
        issue_warnings = False if delete_all else True
        skip_not_found = True if delete_all else False
        return self._delete_resource_by_values(
            resource, skip_not_found=skip_not_found,
            issue_warnings=issue_warnings, **kwargs)
