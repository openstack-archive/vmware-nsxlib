...
class NsxPolicyCertApi(NsxPolicyResourceBase):
    """NSX Policy TrustManagement."""
    @property
    def entry_def(self):
        return policy_defs.CertificateDef
    ...
    def get(self, certificate_id, tenant=policy_constants.POLICY_INFRA_TENANT,
            silent=False):
        certificate_def = self.entry_def(certificate_id=certificate_id,
                                         tenant=tenant)
    ...
    def update(self, certificate_id, name=IGNORE,
               pem_encoded=IGNORE, private_key=IGNORE, description=IGNORE,
               tags=IGNORE, tenant=policy_constants.POLICY_INFRA_TENANT):
