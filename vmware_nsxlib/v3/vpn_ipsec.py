...
class IPSecTunnelProfile(utils.NsxLibApiBase):
    ...
    def create(self, name, description=None,
        ...
        if description:
            body['description'] = description
        if encryption_algorithm:
            body['encryption_algorithm'] = encryption_algorithm
        if digest_algorithm:
        ...
        if dh_group:
            # TODO(asarfaty): string or list?
            body['prefect_forward_secrecy'] = dh_group
        # Boolean parameters
        if pfs is not None:
            body['enable_prefect_forward_secrecy'] = pfs
        if copy_df_bit is not None:
            body['copy_df_bit'] = copy_df_bit
