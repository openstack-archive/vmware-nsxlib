...
class GroupDef(ResourceDef):
    ...
    def update_attributes_in_body(self, body, **kwargs):
        ...
        if kwargs.get('conditions') is not None:
            body['expression'] = kwargs['conditions'].upper()
...
class L4ServiceEntryDef(ResourceDef):
    ...
    def update_attributes_in_body(self, body, **kwargs):
        ...
        if kwargs.get('dest_ports') is not None:
            body['destination_ports'] = kwargs['dest_ports'].upper()
...
class ContractMapDef(ResourceDef):
    def __init__(self,
        ...
        # TODO(asarfaty): support ANY
        self.source_groups = self.get_groups_path(
            domain_id, source_groups) if source_groups else []
        self.dest_groups = self.get_groups_path(
            domain_id, dest_groups) if dest_groups else []
